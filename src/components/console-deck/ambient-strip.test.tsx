...
describe("AmbientStrip", () => {
  it("renders the optimalNext directive in the ambient strip [req:3.1]", () => {
  ...
  it("renders each pending decision as a compact intercom line with kind, title, and a ticking mm:ss TTL countdown [req:3.2]", () => {
  ...
  it("does not use hazard red for a verdict decision whose recommendation starts with Merge [req:3.2]", () => {
    const mergeState = {
  ...
  it("ticks the TTL countdown down as nowMs advances by one second [req:3.2]", () => {
  ...
  it("renders a dimmed NO PENDING DECISIONS line when no decisions are pending [req:3.3]", () => {
  ...
  it("renders a fleet summary with watched count, driver count, and watch queue depth [req:3.4]", () => {
    render(<AmbientStrip state={FIXTURE_STATE_ACTIVE} error={null} nowMs={NOW_MS} />);
  ...
  it("renders a LINK DOWN banner with the error message instead of the state cells when error is set, without throwing [req:3.5]", () => {
    expect(() =>
  ...
  it("still renders the CONSOLE brand cell when the link is down [req:3.5]", () => {
    render(<AmbientStrip state={null} error="fetch failed" nowMs={NOW_MS} />);
