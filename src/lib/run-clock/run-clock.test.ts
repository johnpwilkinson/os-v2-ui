...
describe("parseRunIdStart", () => {
  test("[req:7.1] parses a YYYYMMDDTHHMMSSmmm runId as a local-time epoch", () => {
    const start = parseRunIdStart("20260719T180530657");
    expect(start).toBe(new Date(2026, 6, 19, 18, 5, 30, 657).getTime());
  });

  test("[req:7.1] returns null for a runId that does not match the shape", () => {
    expect(parseRunIdStart("not-a-run-id")).toBeNull();
  });

  test("[req:7.1] returns null for a runId missing the millisecond digits", () => {
    expect(parseRunIdStart("20260719T180530")).toBeNull();
  });
});

describe("formatClock", () => {
  test("clamps negative ms to 0:00", () => {
  ...
  test("renders h:mm:ss at or above one hour", () => {
...
describe("stageClocks", () => {
  test("[req:7.4] sums a stage's done/failed legs' ms when nothing is running", () => {
  ...
  test("[req:7.6] counts a completed leg with no ms value as zero, not NaN", () => {
    const stages: StageNode[] = [
  ...
  test("[req:7.5] ticks the stage owning the most recently opened running leg", () => {
    const stages: StageNode[] = [
  ...
  test("[req:7.5] attributes ticking to the last running leg in stage-then-leg order", () => {
    const stages: StageNode[] = [
  ...
  test("returns ticking false everywhere when no leg is running", () => {
